...
const AdminMessages = () => {
  ...
  const fetchMessages = async (currentPage) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/contact?${query.toString()}`, {
        credentials: 'include'
      });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/contact/bulk-delete`, {
        method: 'POST',
        headers: {
